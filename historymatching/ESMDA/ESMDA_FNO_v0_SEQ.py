...
CeDiag = np.ones(len(dObs))*0.01*np.mean(dObs)
...
Ne=300
NeHf = Ne #ensemble members for the High Fidelity foward model
NePx = Ne #ensemble members for the Proxy
l = 0 #which iteration of the ES-MDA are we starting from
RunPrior =True
ESMDA_its = 8
path_case = f'simulations_fixerror_FNOtrained100_m12_PERM_SEQ_Nit{ESMDA_its}_HF{NeHf}_PX{NePx}'
...
for alpha in alphas:
    ...
    if l==0:        
        ...
        if RunPrior:        
        #reading prior from simulated data
