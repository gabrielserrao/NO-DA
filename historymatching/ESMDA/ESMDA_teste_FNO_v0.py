...
path_case = f'ESDMA_MOD_PARALLEL_simulations_Loc_{Localization}_PERM_Nit{ESMDA_its}_HF{NeHf}_PX{NePx}'
...
def pinv(C, method='svd' ,trunc=0.99):
    if method == 'inv':
        return np.linalg.inv(C)
    elif method == 'svd':
        U, d, Vh = np.linalg.svd(C)
        xi = np.cumsum(d/d.sum())
        for i,e in enumerate(xi):
            if e>=trunc:
                break
        return Vh[:i+1,:].T*(1/d[:i+1])@U[:,:i+1].T      
    else:
        print('*** Metodo: '+method+' nao especificado!')
...
for alpha in alphas:
    ...
    Dobs = np.transpose(dObs + alphas[0]*dstd*np.random.randn(Ne,Nd)) 
    ...
    D = compute_observation_matrix(dynDir, Ne, monitoring_positions, variable='Pressure')
    ...
    #DobsD = DPObs - D
    deltaM = np.transpose(MGrid.T-MGrid.mean(axis=1))    
    deltaD = np.transpose(D.T-D.mean(axis=1))     
    K = deltaM@deltaD.T@pinv(deltaD@deltaD.T + alphas[0]*(Ne-1)*Ce, method='svd',trunc=0.99)
    ...
    #compute the objective function by computing the difference between the observed data and the simulated data divided by the standard deviation of the observed data**2
    #write a function to do it in a single line
    
    #MObj[l,:]= (Dobs - D) / (obs_std) #calcDataMismatchObjectiveFunction(dObs[:,np.newaxis], D, Ce)   
    
    
    
    #calcDataMismatchObjectiveFunction(dObs[:,np.newaxis], D, Ce)
